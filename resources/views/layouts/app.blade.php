...
<head>
    ...
    <title>Seal Viking - Marketplace</title>
    ...
    <style>
        /* From Uiverse.io by Subaashbala - Modified to Glowing Gold for Seal Viking Theme */
        .loader {
        ...
        /* Custom SweetAlert2 Viking Theme Styles */
        .custom-swal-popup {
            background: #0b192c !important;
            border: 2px solid #1e3a68 !important;
            border-radius: 1.5rem !important;
            box-shadow: 0 0 25px rgba(234, 88, 12, 0.25) !important;
        ...
        .custom-swal-confirm-btn:hover {
            box-shadow: 0 4px 25px rgba(234, 88, 12, 0.6) !important;
...
<body class="bg-gray-900 text-white font-sans antialiased">
    ...
    <!-- Loader Page Transition Scripts -->
    <script>
        ...
        document.addEventListener('click', function(e) {
            ...
            if (target && target.href && !target.href.startsWith('#') && !target.href.startsWith('javascript') && !target.target && !e.ctrlKey && !e.metaKey && !e.shiftKey) {
                const loader = document.getElementById('page-loader');
                if (loader) {
        ...
        // Global Alert Helpers overriding standard alerts
        window.showAlert = function(title, text, icon = 'success') {
            let iconColor = '#e5c158'; // Gold for success/info
            if (icon === 'error') iconColor = '#ef4444';
            if (icon === 'warning') iconColor = '#f97316';
        ...
        // Override browser default window.alert with smart theme auto-detection
        window.alert = function(message) {
            let icon = 'info';
            let title = 'Notification';
            
            if (message.includes('⚔️') || message.toLowerCase().includes('success') || message.toLowerCase().includes('complete')) {
                icon = 'success';
                title = 'Success';
            } else if (message.includes('❌') || message.toLowerCase().includes('fail') || message.toLowerCase().includes('error')) {
                icon = 'error';
                title = 'Error';
            } else if (message.includes('🛡️') || message.toLowerCase().includes('pending') || message.toLowerCase().includes('warn')) {
                icon = 'warning';
                title = 'Notice';
            }
            
            window.showAlert(title, message, icon);
        };
    </script>

    <!-- Session Success/Error Alert Triggers -->
    @if(session('success'))
        <script>
            window.addEventListener('DOMContentLoaded', () => {
                window.showAlert('Success', "{{ session('success') }}", 'success');
            });
        </script>
    @endif
    ...
    @if(session('error'))
        <script>
            window.addEventListener('DOMContentLoaded', () => {
                window.showAlert('Error', "{{ session('error') }}", 'error');
            });
        </script>
    @endif
    ...
    @if(session('warning'))
        <script>
            window.addEventListener('DOMContentLoaded', () => {
                window.showAlert('Warning', "{{ session('warning') }}", 'warning');
            });
        </script>
    @endif
</body>
